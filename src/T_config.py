...
使用英文 prompt，要仔細、具體、使用專業的建築設計語法。不要超過200個字。只能生成建築物的外觀，要能看到建築整體的高空鳥瞰圖。
Focus on architectural Appearance with photorealistic style. **Add DO NOT generate text.**
...
一定要放:8K, detailed, best quality, architectural rendering.
設計提案: {advice_text}
改進建議:{improvement}\n語言: {llm_output_language}""",
        "lora_generation": """請生成一個適合的 LoRA 權重數值，其數值必須在 0.5 到 1.5 之間。
權重越重（接近 1.5）表示生成結果會更趨於形式固定沒有創意性的造型，但（例如木質）網格構造清晰適合生成具有簍空網格的構造；
權重越輕（接近 0）則生成結果會更具設計發散性但可能失去網格構造或網格不清晰，整體適合生成較為簡約的造型。
...
**優先項目**
設計符合性與合理性： **需確保圖片符合設計提案所述外觀**。結構與造型需合理且符合預期，展現良好的建築設計邏輯。
